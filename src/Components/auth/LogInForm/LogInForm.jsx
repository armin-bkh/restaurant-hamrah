...
const LogInForm = ({ history }) => {
  ...
  const onSubmit = async (values) => {
          try{
            ...
            if(data.find(em => em.id === values.userPassword && em.name === values.userName)){
  ...
  return (
    <form className={`boxShadow p-5 rounded-3xl w-full md:max-w-lg lg:max-w-xl`} onSubmit={formik.handleSubmit}>
      <h1
      ...
      <button
        type="submit"
        className={`mt-9 lg:mt-14 block w-full py-2 rounded-md Casablanca tracking-widest text-lg  ${formik.isValid ? 'gradient text-white' : 'text-blue-400 border-2 border-blue-400 opacity-40' }`}
